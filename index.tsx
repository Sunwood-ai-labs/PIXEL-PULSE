...
const App = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (imgData.frames && imgData.frames.length > 1) {
        ...
        rgbaFrames.forEach((rgba: ArrayBuffer, idx: number) => {
  ...
  const handleMouseUp = () => {
    if (isDragging && selection) {
      ...
      if (fw > 1 && fh > 1) {
        const id = generateId();
        setFrames(p => [...p, { id, x: Math.floor(selection.width < 0 ? selection.x + selection.width : selection.x), y: Math.floor(selection.height < 0 ? selection.y + selection.height : selection.y), width: Math.floor(fw), height: Math.floor(fh) }]);
  ...
  // Sync canvas with images (including Onion Skin in Workspace)
  useEffect(() => {
    ...
    // Onion Skin in Workspace: Draw the previous frame of the selected one faintly
    if (onionSkin && selectedFrameId) {
      ...
      if (selectedIndex > 0) {
        ...
        ctx.setLineDash([4, 4]);
        ctx.strokeRect(prev.x * zoom, prev.y * zoom, prev.width * zoom, prev.height * zoom);
        ctx.setLineDash([]);
        ctx.globalAlpha = 1.0;
      }
    }

    // Draw all frame rects
    frames.forEach((f, i) => {
  ...
  // Mini preview and full preview sync (including Onion Skin in Preview)
  const renderToCanvas = (canvas: HTMLCanvasElement | null) => {
    ...
    // Onion Skin in Preview: Draw the frame BEFORE the current one faintly
    if (onionSkin && frames.length > 1) {
      const pIdx = (fIdx - 1 + frames.length) % frames.length;
      const pf = frames[pIdx];
      ctx.globalAlpha = 0.3;
      if (pf.dataUrl) {
        const pImg = new Image(); pImg.src = pf.dataUrl;
        ctx.drawImage(pImg, dx, dy, pf.width * s, pf.height * s);
      } else if (image) {
        ctx.drawImage(image, Math.floor(pf.x), Math.floor(pf.y), Math.floor(pf.width), Math.floor(pf.height), dx, dy, pf.width * s, pf.height * s);
      }
      ctx.globalAlpha = 1.0;
    }
    ...
    // Draw current frame
    if (f.dataUrl) {
      const frameImg = new Image(); frameImg.src = f.dataUrl;
      ctx.drawImage(frameImg, dx, dy, f.width * s, f.height * s);
    } else if (image) {
      ctx.drawImage(image, Math.floor(f.x), Math.floor(f.y), Math.floor(f.width), Math.floor(f.height), dx, dy, f.width * s, f.height * s);
    }
  };

  useEffect(() => { renderToCanvas(previewCanvasRef.current); }, [image, frames, currentPreviewFrame, onionSkin]);
  useEffect(() => { if (previewMode) renderToCanvas(fullPreviewCanvasRef.current); }, [image, frames, currentPreviewFrame, previewMode, zoom, onionSkin]);
  ...
  const exportApng = async () => {
    ...
    try {
      ...
      const renderPromises = frames.map(async (f) => {
        ...
        if (f.dataUrl) {
          const frameImg = await new Promise<HTMLImageElement>(res => {
            const i = new Image(); i.onload = () => res(i); i.src = f.dataUrl!;
          });
          fctx.drawImage(frameImg, 0, 0, gw, gh);
        } else if (image) {
    ...
    <div className="flex h-screen w-full bg-[#070708] text-slate-200 overflow-hidden font-sans">
      ...
      <aside className="w-16 border-r border-zinc-800 bg-[#0f0f12] flex flex-col items-center py-6 space-y-6 z-20 shadow-2xl">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <button onClick={() => setOnionSkin(!onionSkin)} className={`p-3 rounded-xl transition-all ${onionSkin ? 'bg-purple-600/20 text-purple-400' : 'hover:bg-zinc-800 text-zinc-400'}`} title="Toggle Onion Skin"><Ghost size={20} /></button>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        <header className="h-14 border-b border-zinc-800 flex items-center justify-between px-6 bg-[#0f0f12]/90 backdrop-blur-md z-10">
          ...
          <div className="flex gap-3">
             <button 
              ...
              className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-5 py-2 rounded-lg text-xs font-bold tracking-widest transition-all"
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-gradient-to-br from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 disabled:from-zinc-800 px-5 py-2 rounded-lg text-xs font-black tracking-widest shadow-xl transition-all active:scale-95"
            >
              <Download size={16} />
        ...
        <div ref={workspaceRef} className="flex-1 overflow-auto bg-[radial-gradient(#1a1a1e_1px,transparent_1px)] [background-size:20px_20px] p-16 flex justify-center items-start">
          {image ? (
            <div 
              className="relative shadow-2xl border border-zinc-800 bg-[#0f0f12] transparency-grid"
              onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}
              style={{ width: image.width * zoom, height: image.height * zoom, cursor: draggingFrameId ? 'grabbing' : 'crosshair' }}
            >
      ...
      <aside className="w-80 border-l border-zinc-800 bg-[#0f0f12] flex flex-col shadow-2xl z-20">
        <div className="p-6 border-b border-zinc-800">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-500 flex items-center gap-2">
              Preview {onionSkin && <span className="text-[8px] bg-purple-900/40 px-1 text-purple-400 rounded">Onion ON</span>}
            </h3>
            <button onClick={() => setIsPlaying(!isPlaying)} className={`p-1.5 rounded-lg transition-all ${isPlaying ? 'text-purple-400 bg-purple-900/20' : 'text-zinc-500 hover:bg-zinc-800'}`}>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="px-6 py-4 border-b border-zinc-800 bg-zinc-900/30">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-500 flex items-center gap-2">
              <Layers size={14} /> Sequence ({frames.length})
            </h3>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {frames.map((f, idx) => (
              <div 
                ...
                className={`group flex items-center gap-4 p-3 rounded-2xl border cursor-pointer transition-all ${selectedFrameId === f.id ? 'bg-purple-900/10 border-purple-500/30' : 'bg-zinc-900/20 border-transparent hover:bg-zinc-800/40'}`}
              >
                <div className="w-14 h-14 bg-zinc-950 rounded-xl border border-zinc-800 flex items-center justify-center overflow-hidden shadow-inner transparency-grid">
                   {f.dataUrl ? (
                     <img src={f.dataUrl} className="rendering-pixelated w-full h-full object-contain" />
                   ) : image && (
                    <div className="rendering-pixelated" style={{ backgroundImage: `url(${image.src})`, backgroundPosition: `-${Math.floor(f.x)}px -${Math.floor(f.y)}px`, width: Math.floor(f.width), height: Math.floor(f.height), transform: `scale(${Math.min(48/f.width, 48/f.height)})`, transformOrigin: 'top left' }} />
                   )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] font-black uppercase tracking-tight ${idx === currentPreviewFrame ? 'text-purple-400' : 'text-zinc-300'}`}>Frame {idx + 1}</p>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-[#070708] flex flex-col">
          <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-8 bg-[#0f0f12]">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400">Live Animation Preview</h2>
              {onionSkin && <span className="text-[10px] bg-purple-600 px-2 py-0.5 text-white rounded font-bold">Onion Skin Active</span>}
            </div>
            <div className="flex items-center gap-6">
              <div className="flex bg-zinc-900 rounded-lg p-1 border border-zinc-800">
                {(['grid', 'black', 'white', 'green'] as const).map(bg => (
                  <button 
                    key={bg} 
                    onClick={() => setPreviewBg(bg)}
                    className={`px-3 py-1.5 rounded text-[9px] font-bold uppercase transition-all ${previewBg === bg ? 'bg-purple-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                  >
                    {bg}
                  </button>
                ))}
              </div>
              <button onClick={() => setPreviewMode(false)} className="p-2 hover:bg-zinc-800 rounded-full text-zinc-500 hover:text-white transition-all">
                <X size={24} />
              </button>
            </div>
          </header>
          
          <div className={`flex-1 flex items-center justify-center overflow-hidden transition-colors ${
            previewBg === 'grid' ? 'transparency-grid' : 
            previewBg === 'black' ? 'bg-black' : 
            previewBg === 'white' ? 'bg-white' : 'bg-[#00ff00]'
          }`}>
          ...
          <footer className="h-16 border-t border-zinc-800 bg-[#0f0f12] flex items-center justify-center px-8 gap-12">
            <div className="flex items-center gap-3">
              <button onClick={() => setIsPlaying(!isPlaying)} className="p-3 bg-purple-600 rounded-full text-white hover:bg-purple-500 transition-all shadow-lg shadow-purple-600/20">
                {isPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" />}
              </button>
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase text-zinc-500 leading-none mb-1">Status</span>
                <span className="text-[10px] font-bold text-zinc-200 uppercase tracking-widest flex items-center gap-1">
                  {isPlaying ? 'Looping' : 'Paused'} <CheckCircle2 size={12} className="text-purple-400" />
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
               <span className="text-[9px] font-black uppercase text-zinc-500">Speed</span>
               <div className="flex items-center gap-3 bg-zinc-900 px-4 py-2 rounded-xl border border-zinc-800">
                 <input type="range" min="1" max="60" value={fps} onChange={e => setFps(parseInt(e.target.value))} className="w-32 h-1 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                 <span className="text-[10px] font-mono font-bold text-zinc-400 min-w-[3ch]">{fps}</span>
               </div>
            </div>
            ...
            <button 
              onClick={exportApng}
              disabled={exporting}
              className="ml-auto flex items-center gap-2 bg-purple-600 hover:bg-purple-500 px-6 py-2.5 rounded-xl text-xs font-black tracking-widest transition-all shadow-xl shadow-purple-600/10"
            >
              <Download size={16} /> DOWNLOAD FINAL APNG
            </button>
          </footer>
        </div>
      )}
